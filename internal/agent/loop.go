...
func (a *AgentLoop) runToolLoop(
	...
	// Build the eino model adapter wrapping the EazyClaw provider.
	modelAdapter := newEinoModelAdapter(prov, modelName)

	// Build eino tool adapters from the registry.
	einoTools := wrapToolsAsBaseTool(a.tools)

	// Build the ToolsNodeConfig with the adapted tools.
	toolsConfig := compose.ToolsNodeConfig{
		Tools: einoTools,
	}

	// The MessageModifier injects the runtime snapshot into the system prompt
	// each time the model is called (similar to the per-iteration logic in the old loop).
	iterationCounter := 0
	messageModifier := func(_ context.Context, input []*schema.Message) []*schema.Message {
		iterationCounter++
		snapshot := buildRuntimeSnapshotPrompt(
			prov.Name(),
			modelName,
			contextWindowTokens,
		...
		fullPrompt := systemPrompt + snapshot
		out := make([]*schema.Message, 0, len(input)+1)
		out = append(out, schema.SystemMessage(fullPrompt))
		// Include only non-system messages from input (system is now prepended).
		for _, m := range input {
	...
	// Create the eino ReAct agent.
	// MaxStep is the total number of graph steps (node count + iterations).
	// The react agent has 2 main nodes (model + tools), so maxSteps = 2 + maxIterations*2.
	agent, err := react.NewAgent(ctx, &react.AgentConfig{
		ToolCallingModel: modelAdapter,
		ToolsConfig:      toolsConfig,
		MaxStep:          2 + maxIterations*2,
		MessageModifier:  messageModifier,
	})
	if err != nil {
		return history, lastUsage, fmt.Errorf("eino: failed to create react agent: %w", err)
	}

	// Convert existing history to eino messages as input.
	inputMsgs := toEinoMessages(history)

	// Use WithMessageFuture to collect all intermediate messages (assistant + tool results).
	futureOpt, future := react.WithMessageFuture()

	// Run the agent.
	finalMsg, err := agent.Generate(ctx, inputMsgs, futureOpt)
	if err != nil {
		return history, lastUsage, err
	}

	// Collect all intermediate messages generated by the agent.
	iter := future.GetMessages()
	for {
		msg, ok, iterErr := iter.Next()
		if iterErr != nil {
			slog.Warn("agent: eino message future error", "error", iterErr)
			break
		}
		if !ok {
			break
		}
		pm := fromEinoMessage(msg)
		if pm.Role == "assistant" && len(msg.ToolCalls) > 0 {
			for _, tc := range msg.ToolCalls {
				slog.Info("agent: executing tool (via eino)", "tool", tc.Function.Name, "call_id", tc.ID)
			}
		}
		// Extract usage from the final assistant message's ResponseMeta.
		if msg.ResponseMeta != nil && msg.ResponseMeta.Usage != nil {
			lastUsage = providerPkg.Usage{
	...
	// If the final message was not already captured by the future (it should be
	// the last assistant message), ensure it's in the history.
	if finalMsg != nil {
		finalContent := strings.TrimSpace(finalMsg.Content)
		// Check if the last message in history already matches.
		...
		if !alreadyCaptured && finalContent != "" {
			history = append(history, fromEinoMessage(finalMsg))
		}
		if finalMsg.ResponseMeta != nil && finalMsg.ResponseMeta.Usage != nil {
			lastUsage = providerPkg.Usage{
				InputTokens:  finalMsg.ResponseMeta.Usage.PromptTokens,
				OutputTokens: finalMsg.ResponseMeta.Usage.CompletionTokens,
			}
		}

		// Send reply to user if applicable.
		if doSendReply && origin != nil && finalContent != "" && !strings.EqualFold(finalContent, noReplyToken) {
			a.sendReply(*origin, finalMsg.Content)
		}
	}
